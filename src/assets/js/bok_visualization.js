...
displayBOK = function (bokData) {
  ...
  var nodes = pack.radius(5).nodes(root).sort((a, b) => b.nameShort - a.nameShort);
  var view;
  ...
  var circle = svg.selectAll("circle").data(nodes).enter().append("circle").attr("id", function (d) {
      ...
      if (d.depth == 1) {
        return colorPalette(dataAndFunctions.colorhash[d.nameShort.substring(1, 3)]);
      } else if (d.depth == 2) {
        return colorPalette(dataAndFunctions.colorhash[d.parent.nameShort.substring(1, 3)]);
      } else if (d.depth >= 3) {
        return colorPalette(dataAndFunctions.colorhash[d.parent.parent.nameShort.substring(1, 3)]);
      } else {
  ...
  dataAndFunctions.zoom = function (d) {
    ...
    }).each("start", function (d) {}).style("display", function (d) {
...
displayConcept = function (d, namehash) {
  ...
  document.getElementById("headline").innerHTML = d.name;
  ...
  displayOrderedList(d.children, "name", "Subconcepts", domElement, namehash, d);
...
createLabels = function (nodes, root) {
  var text = svg.selectAll("text").data(nodes).enter().append("text").attr("class", "label").style("pointer-events", "none").style("fill-opacity", function (d) {
    return d.parent === root || (d === root && d.children == null) ? 0.8 : 0;
