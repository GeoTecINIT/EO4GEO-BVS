...
parseBOKDataJSON = function (bokJSON) {
  ...
  for (var n = 0; n < bokJSON.nodes.length; n++) {
    ...
    newNode.name = newNode.nameShort = bokJSON.nodes[n].label;
  ...
  for (var l = 0; l < bokJSON.links.length; l++) {
    ...
    if (bokJSON.links[l].relationName == "is subconcept of") {
  ...
  return {
    ...
    namehash: namehash,
    conceptNodeCollection: allNodes,
    colors: colorhash
  };
};
